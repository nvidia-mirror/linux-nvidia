...
static int nvvc_hv_init(nvvc_dev_t *nvvcdev)
{
	...
	for (i = 0; i < s_infop->nr_queues; i++) {
		...
		if (qd->is_virtio_queue != 0) {
			nvvcdev->ivc_id = qd->id;
			pr_debug("nvvc: ivc queue id: 0x%x\n", nvvcdev->ivc_id);
			break;
		}
	}

	for (i = 0; i < s_infop->nr_mempools; i++) {
		const struct ivc_mempool *mpd =
				&ivc_info_mempool_array(s_infop)[i];
		if (mpd->is_virtio_mempool != 0) {
			nvvcdev->ivm_id = mpd->id;
			pr_debug("nvvc: ivc mempool id: 0x%x\n", nvvcdev->ivm_id);
			break;
		}
	}
#else
	(void)i;
	pr_err("nvvc: enable virtio support in ivc queue and mempool\n");
	BUG();
#endif
